...
# FIX 2: Add btn.classList.add('is-open') at the end of openTaskMoreFloat
...
# FIX 3: Update toggleTaskMoreMenu to not call rT()
...
# FIX 5: Remove rT() from document click handler (first occurrence)
$old5a = "    if (typeof taskMoreMenuId === 'undefined' || taskMoreMenuId == null) return;
...
$new5a = "    if (typeof taskMoreMenuId === 'undefined' || taskMoreMenuId == null) return;
    if (e.target.closest && (e.target.closest('.task-more-wrap') || e.target.closest('.task-more-float'))) return;
    if (typeof closeTaskMoreFloat === 'function') closeTaskMoreFloat();
    taskMoreMenuId = null;
  }, true);
