...
function syncFilterSegIndicator(segEl){if(!segEl||!segEl.querySelector)return;var activeBtn=segEl.querySelector(".filter-seg-btn.is-active")||segEl.querySelector(".filter-seg-btn[aria-selected=\"true\"]");if(!activeBtn){segEl.classList.remove("filter-seg--indicator");segEl.style.setProperty("--seg-indicator-o","0");return}var segRect=segEl.getBoundingClientRect();var btnRect=activeBtn.getBoundingClientRect();var firstBtn=segEl.querySelector(".filter-seg-btn");var baseLeft=0;if(firstBtn){var firstRect=firstBtn.getBoundingClientRect();baseLeft=Math.round(firstRect.left-segRect.left)}var x=Math.max(0,Math.round(btnRect.left-segRect.left)-baseLeft);var w=Math.max(0,Math.round(btnRect.width));segEl.classList.add("filter-seg--indicator");segEl.style.setProperty("--seg-indicator-x",x+"px");segEl.style.setProperty("--seg-indicator-w",w+"px");segEl.style.setProperty("--seg-indicator-o",w>0?"1":"0")}
function rFilterBar(){
  ...
  if(segEl){
    requestAnimationFrame(function(){syncFilterSegIndicator(segEl)});
  }
