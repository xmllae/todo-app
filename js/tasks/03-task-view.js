...
function taskOverdueMetaHtml(titleText){const tip=titleText||"\u8be5\u4efb\u52a1\u5df2\u903e\u671f";return'<span class="week-task-overdue-meta" title="'+esc(tip)+'">'+WEEK_OVERDUE_ICON+'<span>\u5df2\u903e\u671f</span></span>'}
...
function renderOverdueTaskOverviewSidebar(state){
  ...
  shell.innerHTML='<div class="week-overview-head overdue-overview-head"><span class="week-overview-kicker">\u903e\u671f\u603b\u89c8</span></div><div class="overdue-overview-hero"><div class="overdue-overview-count"><div class="overdue-overview-count-badge"><strong>'+state.visibleCount+'</strong></div><div class="overdue-overview-count-main"><span class="overdue-overview-count-copy">\u5f53\u524d\u5f85\u5904\u7406</span>'+summaryCopyHtml+'</div></div><div class="task-overview-metrics overdue-overview-metrics">'+metricsHtml+'</div></div><div class="overdue-overview-section"><div class="overdue-overview-section-head"><div class="overdue-overview-section-title">\u6700\u8fd1\u903e\u671f\u65e5\u671f</div></div><div class="overdue-overview-list">'+(listHtml||'<div class="overdue-overview-empty">\u5f53\u524d\u6ca1\u6709\u9700\u8981\u5904\u7406\u7684\u903e\u671f\u4efb\u52a1\u3002</div>')+'</div></div>'
}
...
function renderWeekTaskScene(list,baseDs){
  ...
  const cardList=meta.days.map(function(ds,idx){
    ...
    const todoControl=isExpandable?'<button type="button" class="week-day-count-chip week-day-count-chip--todo'+todoToneCls+' week-todo-count-toggle'+(isExpanded?" is-open":"")+'" data-hidden-count="'+hiddenCount+'" data-preview-count="'+previewRows.length+'" data-todo-count="'+dayTodoCount+'" data-total-count="'+totalDayRows+'" data-count-label="'+countTip+'" aria-expanded="'+(isExpanded?"true":"false")+'" aria-label="'+countTip+'\uff0c'+(isExpanded?"\u6536\u8d77 ":"\u67e5\u770b ")+hiddenCount+' \u9879\u5269\u4f59\u4efb\u52a1" title="'+countTip+'\uff0c'+(isExpanded?"\u6536\u8d77 ":"\u67e5\u770b ")+hiddenCount+' \u9879\u5269\u4f59\u4efb\u52a1" onclick="event.stopPropagation();toggleWeekDayExpand(\''+ds+'\')"><span class="week-task-expand-label"><span class="week-day-count-line week-day-count-line--todo'+todoLineCls+'">'+todoIcon+'<span>'+todoLabel+'</span><b>'+todoCountText+'</b></span></span><span class="week-day-count-caret" aria-hidden="true"></span></button>':'<span class="week-day-count-chip week-day-count-chip--todo'+todoToneCls+'" title="'+todoLabel+' '+dayTodoCount+' \u9879"><span class="week-day-count-line week-day-count-line--todo'+todoLineCls+'">'+todoIcon+'<span>'+todoLabel+'</span><b>'+dayTodoCount+'</b></span><span class="week-day-count-caret is-placeholder" aria-hidden="true"></span></span>';
