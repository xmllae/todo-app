...
const subTitleSuffix=subT>0?`<button type="button" class="sub-task-pill-btn sub-task-pill sub-task-pill--inline${subAllDone?" sub-task-pill--all-done":""}${isExp?" sub-task-pill--open":""}" title="\u5b50\u4efb\u52a1 ${subD}/${subT}" aria-expanded="${isExp?"true":"false"}" aria-label="\u5b50\u4efb\u52a1 ${subD}/${subT}" onclick="event.stopPropagation();toggleExpand(${t.id})">${subAllDone?'<span class="stp-strike-line" data-strike-animate="1" aria-hidden="true"></span>':""}<span class="stp-icon" aria-hidden="true"></span><span class="stp-n">${subD} / ${subT}</span></button>`:"";
...
subRows+=`<div class="subtask-item subtask-item--editing"><button type="button" class="sub-ck ${s.done?"checked":""}" aria-label="${subToggleAria}" onclick="event.stopPropagation();toggleSubtask(${t.id},${s.id})"></button><input class="sub-text-edit" id="subEdit_${s.id}" value="${esc(s.text)}" onkeydown="if(event.key==='Enter')saveEditSub(${t.id},${s.id})" onblur="setTimeout(()=>saveEditSub(${t.id},${s.id}),120)"><div class="sub-tail"><button type="button" class="sub-del" title="\u5220\u9664\u5b50\u4efb\u52a1" aria-label="\u5220\u9664\u5b50\u4efb\u52a1" onclick="event.stopPropagation();deleteSubtask(${t.id},${s.id})">&times;</button></div></div>`
}else{
subRows+=`<div class="subtask-item${s.done?" subtask-item--done":""}" onclick="event.stopPropagation();toggleSubtask(${t.id},${s.id})"><button type="button" class="sub-ck ${s.done?"checked":""}" aria-label="${subToggleAria}" onclick="event.stopPropagation();toggleSubtask(${t.id},${s.id})"></button><div class="sub-main"><span class="sub-text ${s.done?"sub-done":""}" ondblclick="event.stopPropagation()">${esc(s.text)}</span></div><div class="sub-tail">${subStateH}<button type="button" class="sub-del" title="\u5220\u9664\u5b50\u4efb\u52a1" aria-label="\u5220\u9664\u5b50\u4efb\u52a1" onclick="event.stopPropagation();deleteSubtask(${t.id},${s.id})">&times;</button></div></div>`
}
});
const subAddRow=`<div class="sub-add-wrap sub-add-wrap--list" onclick="event.stopPropagation()"><div class="sub-add-form${subAddOpen?"":" sub-add-form--hidden"}" role="group" aria-label="\u65b0\u5efa\u5b50\u4efb\u52a1"><div class="sub-ck sub-add-ck-ph" aria-hidden="true"></div><input type="text" class="sub-add-input" id="subAdd_${t.id}" placeholder="\u8f93\u5165\u5b50\u4efb\u52a1\u5185\u5bb9\uff0c\u6309\u56de\u8f66\u4fdd\u5b58" autocomplete="off" onclick="event.stopPropagation()" onkeydown="if(event.key==='Enter'){event.preventDefault();addSubtask(${t.id})}else if(event.key==='Escape'){event.preventDefault();cancelSubAddCompose(${t.id})}"><div class="sub-add-actions"><button type="button" class="sub-add-cancel" title="Esc" onclick="event.stopPropagation();cancelSubAddCompose(${t.id})">\u53d6\u6d88</button><button type="button" class="sub-add-save" title="Enter" onclick="event.stopPropagation();addSubtask(${t.id})">\u4fdd\u5b58</button></div></div></div>`;
expandH=`<div class="task-expand-area task-expand-area--tabbed task-expand-area--open"><div class="task-expand-drop"><div class="task-exp-sub-bg"><div class="exp-block exp-block--sub"><div class="subtask-section-head" onclick="event.stopPropagation()"><div class="subtask-section-meta"><span class="subtask-section-title">\u5b50\u4efb\u52a1</span></div><button type="button" class="subtask-section-add${subAddOpen?" is-active":""}" title="${subAddOpen?"\u53d6\u6d88\u5b50\u4efb\u52a1":"\u6dfb\u52a0\u5b50\u4efb\u52a1"}" aria-label="${subAddOpen?"\u53d6\u6d88\u5b50\u4efb\u52a1":"\u6dfb\u52a0\u5b50\u4efb\u52a1"}" aria-expanded="${subAddOpen?"true":"false"}" onclick="event.stopPropagation();toggleSubAddCompose(${t.id})"><span class="subtask-section-add-plus" aria-hidden="true"></span><span class="subtask-section-add-label">${subAddOpen?"\u53d6\u6d88\u5b50\u4efb\u52a1":"\u6dfb\u52a0\u5b50\u4efb\u52a1"}</span></button></div><div class="subtask-list">${subRows}</div>${subAddRow}</div></div></div></div>`
}
...
function animateSubtaskStrikeLines(){const root=document.getElementById("tList");if(!root)return;const lines=root.querySelectorAll('.stp-strike-line[data-strike-animate="1"]:not([data-strike-animated="1"])');if(!lines.length)return;lines.forEach(function(line){line.dataset.strikeAnimated="1";if(typeof line.animate==="function"){line.style.transform="translateY(-50%) scaleX(1)";line.style.opacity="1";line.animate([{transform:"translateY(-50%) scaleX(0)",opacity:.76},{transform:"translateY(-50%) scaleX(1)",opacity:1}],{duration:420,easing:"cubic-bezier(0.22, 1, 0.36, 1)",fill:"forwards"});return}line.style.transition="none";line.style.transform="translateY(-50%) scaleX(0)";line.style.opacity=".76";void line.offsetWidth;line.style.transition="transform 420ms cubic-bezier(0.22, 1, 0.36, 1), opacity 420ms cubic-bezier(0.22, 1, 0.36, 1)";line.style.transform="translateY(-50%) scaleX(1)";line.style.opacity="1"})}
