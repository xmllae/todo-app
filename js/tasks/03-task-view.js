...
const isExp=expandedId===t.id||showSubtasksByDefault&&subT>0;
...
expandH=`<div class="task-expand-area task-expand-area--tabbed task-expand-area--open"><div class="task-expand-drop"><div class="task-exp-sub-bg"><div class="exp-block exp-block--sub"><div class="subtask-section-head" onclick="event.stopPropagation()"><div class="subtask-section-meta"><span class="subtask-section-title">\u5b50\u4efb\u52a1</span><span class="subtask-section-count">${subD} / ${subT}</span></div><button type="button" class="subtask-section-add${subAddOpen?" is-active":""}" aria-expanded="${subAddOpen?"true":"false"}" onclick="event.stopPropagation();openSubAddCompose(${t.id})"><span class="subtask-section-add-plus" aria-hidden="true">+</span><span>\u6dfb\u52a0\u5b50\u4efb\u52a1</span></button></div><div class="subtask-list">${subRows}</div>${subAddRow}</div></div></div></div>`
}
