...
function setWeekDayExpanded(ds,isOpen){
  const card=getWeekDayCard(ds),extra=card?card.querySelector(".week-task-extra"):null,btn=card?card.querySelector(".week-todo-count-toggle"):null,taskList=card?card.querySelector(".week-task-list"):null,total=card?Number(card.getAttribute("data-week-task-count")||0):0;
...
function renderWeekTaskScene(list,baseDs){
  ...
  const cardList=meta.days.map(function(ds,idx){
    ...
    const d=parseDS(ds),md=d.getMonth()+1+"\u6708"+d.getDate()+"\u65e5",dayLabel="\u5468"+weekNames[idx]+" "+md,statusTxt=dayPending?"":raw.length?"\u5df2\u5168\u90e8\u5b8c\u6210":"",dateTxt='<span class="week-day-date-text">'+md+'</span>',overdueTxt=isOverdueDay?'<span class="week-day-overdue-text">\u903e\u671f '+dayPending+'</span>':"",titleMeta=dateTxt+(overdueTxt?'<span class="week-day-meta-dot">\u00b7</span>'+overdueTxt:statusTxt?'<span class="week-day-meta-dot">\u00b7</span><span class="week-day-status-text">'+statusTxt+'</span>':""),hasTaskBody=dayRows.length||dayDoneRows.length,isClearDay=!dayPending&&raw.length>0,cls="week-day-card"+(ds===todayDs?" is-today":"")+(ds===baseDs?" is-focus":"")+(isOverdueDay?" is-overdue":"")+(isClearDay?" is-clear":"")+(showDayDone?" is-done-open":"")+(hasTaskBody?"":" is-empty"),previewMax=WEEK_DAY_PREVIEW_MAX,isExpandable=dayRows.length>previewMax,isExpanded=isExpandable&&isWeekDayExpanded(ds),previewRows=dayRows.slice(0,previewMax),extraRows=isExpandable?dayRows.slice(previewMax):[],hiddenCount=extraRows.length,listCls="week-task-list"+(isExpanded&&dayRows.length>10?" is-scroll":"")+(hasTaskBody?"":" is-empty");
    ...
    const rowsHtml=previewRows.map(renderWeekRow).join("")+extraHtml+doneSlideHtml;
  ...
  const dayStripHtml='<div class="week-view-day-strip" aria-label="\u672c\u5468\u6bcf\u65e5\u5f85\u529e\u6982\u89c8"><div class="week-view-day-pills">'+dailyOverview.map(function(day){const name=weekNames[day.idx],state=day.isOverdue?"\u903e\u671f "+day.pending+" \u9879\u5f85\u529e":day.pending?day.pending+" \u9879\u5f85\u529e":day.total?"\u5df2\u5168\u90e8\u5b8c\u6210":"\u6682\u65e0\u4efb\u52a1",cls="week-view-day-pill"+(day.isToday?" is-today":"")+(day.isOverdue?" is-overdue":"")+(day.pending?" has-pending":day.total?" is-clear":" is-empty");return'<button type="button" class="'+cls+'" aria-label="\u5b9a\u4f4d\u5230\u5468'+name+'\uff0c'+state+'" title="\u5468'+name+'\uff1a'+state+'" onclick="jumpWeekDay(\''+day.ds+'\')"><span class="week-view-day-pill-name">\u5468'+name+'</span><span class="week-view-day-pill-count">'+day.pending+'</span></button>'}).join("")+'</div></div>';
