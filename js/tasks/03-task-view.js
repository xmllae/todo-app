...
const doneToggleLabel=collapseCompletedRows?`\u663e\u793a\u5df2\u5b8c\u6210 ${subD}`:`\u6298\u53e0\u5df2\u5b8c\u6210 ${subD}`;
const doneToggleBtn=hasDoneRows?`<button type="button" class="subtask-completed-toggle${collapseCompletedRows?" is-collapsed":""}" aria-expanded="${collapseCompletedRows?"false":"true"}" aria-label="${doneToggleLabel}" title="${doneToggleLabel}" onclick="event.stopPropagation();toggleCompletedSubtasksCollapse(${t.id})"><span class="subtask-completed-toggle-label">${doneToggleLabel}</span><span class="subtask-completed-toggle-chev" aria-hidden="true"></span></button>`:"";
const doneRowsHtml=hasDoneRows?`<div class="subtask-completed-wrap${collapseCompletedRows?" is-collapsed":""}">${subRowsDone}</div>`:"";
const doneHiddenHint=hasDoneRows&&collapseCompletedRows?`<div class="subtask-completed-hidden-hint" onclick="event.stopPropagation();toggleCompletedSubtasksCollapse(${t.id})">已有${subD}个子任务完成后被折叠</div>`:"";
...
expandH=`<div class="task-expand-area task-expand-area--tabbed task-expand-area--open"><div class="task-expand-drop"><div class="task-exp-sub-bg"><div class="exp-block exp-block--sub"><div class="subtask-section-head" onclick="event.stopPropagation()"><div class="subtask-section-meta"><span class="subtask-section-title">\u5b50\u4efb\u52a1</span></div><div class="subtask-section-actions">${doneToggleBtn}<button type="button" class="subtask-section-add${subAddOpen?" is-active":""}" title="${subAddOpen?"\u53d6\u6d88\u5b50\u4efb\u52a1":"\u6dfb\u52a0\u5b50\u4efb\u52a1"}" aria-label="${subAddOpen?"\u53d6\u6d88\u5b50\u4efb\u52a1":"\u6dfb\u52a0\u5b50\u4efb\u52a1"}" aria-expanded="${subAddOpen?"true":"false"}" onclick="event.stopPropagation();toggleSubAddCompose(${t.id})"><span class="subtask-section-add-plus" aria-hidden="true"></span><span class="subtask-section-add-label">${subAddOpen?"\u53d6\u6d88\u5b50\u4efb\u52a1":"\u6dfb\u52a0\u5b50\u4efb\u52a1"}</span></button></div></div><div class="subtask-list">${subRowsTodo}${doneRowsHtml}${doneHiddenHint}</div>${subAddRow}</div></div></div></div>`
}
