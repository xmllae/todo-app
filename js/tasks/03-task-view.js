...
function toggleWeekDoneCollapse(){
  ...
  slides.forEach(function(slide){
    ...
    animateWeekDoneSlide(slide,showDone,height)
  })
}
...
function setWeekDayExpanded(ds,isOpen){
  ...
  if(hint){
    ...
    hint.setAttribute("aria-hidden",isOpen?"true":"false");
    hint.tabIndex=isOpen?-1:0
  }
}
...
function renderWeekTaskScene(list,baseDs){
  ...
  const cardList=meta.days.map(function(ds,idx){
    ...
    dailyOverview.push({ds:ds,idx:idx,pending:dayPending,done:dayDone,total:raw.length,isToday:ds===todayDs,isOverdue:isOverdueDay});
    ...
    const d=parseDS(ds),md=d.getMonth()+1+"\u6708"+d.getDate()+"\u65e5",dayLabel="\u5468"+weekNames[idx]+" "+md,statusTxt=dayPending?"":raw.length?"\u5df2\u5168\u90e8\u5b8c\u6210":"",dateTxt='<span class="week-day-date-text">'+md+'</span>',overdueTxt=isOverdueDay?'<span class="week-day-overdue-text"><span class="week-overdue-mark" aria-hidden="true">\u26a0</span><span>\u903e\u671f '+dayPending+'</span></span>':"",titleMeta=dateTxt+(overdueTxt?'<span class="week-day-meta-dot">\u00b7</span>'+overdueTxt:statusTxt?'<span class="week-day-meta-dot">\u00b7</span><span class="week-day-status-text">'+statusTxt+'</span>':""),hasTaskBody=dayRows.length||dayDoneRows.length,isClearDay=!dayPending&&raw.length>0,cls="week-day-card"+(ds===todayDs?" is-today":"")+(ds===baseDs?" is-focus":"")+(isOverdueDay?" is-overdue":"")+(isClearDay?" is-clear":"")+(showDayDone?" is-done-open":"")+(hasTaskBody?"":" is-empty"),previewMax=WEEK_DAY_PREVIEW_MAX,isExpandable=dayRows.length>previewMax,isExpanded=isExpandable&&isWeekDayExpanded(ds),previewRows=dayRows.slice(0,previewMax),extraRows=isExpandable?dayRows.slice(previewMax):[],hiddenCount=extraRows.length,listCls="week-task-list"+(isExpanded&&dayRows.length>10?" is-scroll":"")+(hasTaskBody?"":" is-empty");
    ...
    const moreHintHtml=isExpandable?'<button type="button" class="week-task-more-hint'+(isExpanded?" is-hidden":"")+'" aria-hidden="'+(isExpanded?"true":"false")+'" tabindex="'+(isExpanded?"-1":"0")+'" onclick="event.stopPropagation();toggleWeekDayExpand(\''+ds+'\')" title="\u5c55\u5f00 '+hiddenCount+' \u9879\u5269\u4f59\u5f85\u529e"><span>\u5269\u4f59 '+hiddenCount+' \u9879\u5f85\u529e</span><b>\u5c55\u5f00</b><span class="week-day-count-caret" aria-hidden="true"></span></button>':"";
    ...
    const rowsHtml=previewRows.map(renderWeekRow).join("")+moreHintHtml+extraHtml+doneSlideHtml;
    const todoControl=isExpandable?'<button type="button" class="week-day-count-chip week-day-count-chip--todo week-todo-count-toggle'+(isExpanded?" is-open":"")+'" data-hidden-count="'+hiddenCount+'" data-total-count="'+totalDayRows+'" data-count-label="'+countTip+'" aria-expanded="'+(isExpanded?"true":"false")+'" aria-label="'+countTip+'\uff0c'+(isExpanded?"\u6536\u8d77 ":"\u67e5\u770b ")+hiddenCount+' \u9879\u5269\u4f59\u4efb\u52a1" title="'+countTip+'\uff0c'+(isExpanded?"\u6536\u8d77 ":"\u67e5\u770b ")+hiddenCount+' \u9879\u5269\u4f59\u4efb\u52a1" onclick="event.stopPropagation();toggleWeekDayExpand(\''+ds+'\')"><span class="week-task-expand-label"><span class="week-day-count-line week-day-count-line--todo"><span>\u5f85\u529e</span><b>'+dayTodoCount+'</b></span></span><span class="week-day-count-caret" aria-hidden="true"></span></button>':'<span class="week-day-count-chip week-day-count-chip--todo" title="\u5f85\u529e '+dayTodoCount+' \u9879"><span class="week-day-count-line week-day-count-line--todo"><span>\u5f85\u529e</span><b>'+dayTodoCount+'</b></span></span>';
    const doneControl=dayDoneRows.length?'<button type="button" class="week-day-count-chip week-day-count-chip--done week-done-count-toggle'+(showDayDone?" is-open":"")+'" data-done-count="'+dayDoneRows.length+'" data-day-label="'+esc(dayLabel)+'" aria-expanded="'+(showDayDone?"true":"false")+'" aria-label="'+(showDayDone?"\u6536\u8d77":"\u67e5\u770b")+dayLabel+' \u7684\u5df2\u5b8c\u6210\u4efb\u52a1" title="'+(showDayDone?"\u6536\u8d77\u5f53\u5929\u5df2\u5b8c\u6210\u4efb\u52a1":"\u67e5\u770b\u5f53\u5929\u5df2\u5b8c\u6210\u4efb\u52a1")+'" onclick="event.stopPropagation();toggleWeekDoneForDay(\''+ds+'\')"><span class="week-day-count-line week-day-count-line--done"><span>\u5df2\u5b8c\u6210</span><b>'+dayDoneCount+'</b></span><span class="week-day-count-caret" aria-hidden="true"></span></button>':'<span class="week-day-count-chip week-day-count-chip--done" title="\u5df2\u5b8c\u6210 '+dayDoneCount+' \u9879"><span class="week-day-count-line week-day-count-line--done"><span>\u5df2\u5b8c\u6210</span><b>'+dayDoneCount+'</b></span></span>';
    const countHtml='<div class="week-day-count week-day-count-group" title="'+countTip+'" aria-label="'+countTip+'">'+todoControl+doneControl+'</div>';
  ...
  const dayStripHtml='<div class="week-view-day-strip" aria-label="\u672c\u5468\u6bcf\u65e5\u5f85\u529e\u6982\u89c8"><div class="week-view-day-pills">'+dailyOverview.map(function(day){const name=weekNames[day.idx],state=day.isOverdue?"\u903e\u671f "+day.pending+" \u9879\u5f85\u529e":day.pending?day.pending+" \u9879\u5f85\u529e":day.total?"\u5df2\u5168\u90e8\u5b8c\u6210":"\u6682\u65e0\u4efb\u52a1",cls="week-view-day-pill"+(day.isToday?" is-today":"")+(day.isOverdue?" is-overdue":"")+(day.pending?" has-pending":day.total?" is-clear":" is-empty"),valueHtml=day.isOverdue?'<span class="week-view-day-pill-alert"><span class="week-overdue-mark" aria-hidden="true">\u26a0</span><span>\u903e\u671f</span><span class="week-view-day-pill-count">'+day.pending+'</span></span>':'<span class="week-view-day-pill-count">'+day.pending+'</span>';return'<button type="button" class="'+cls+'" aria-label="\u5b9a\u4f4d\u5230\u5468'+name+'\uff0c'+state+'" title="\u5468'+name+'\uff1a'+state+'" onclick="jumpWeekDay(\''+day.ds+'\')"><span class="week-view-day-pill-name">\u5468'+name+'</span>'+valueHtml+'</button>'}).join("")+'</div></div>';
  const gridInner=cardList.map(function(item){return item.html}).join("");
  list.innerHTML='<div class="week-view'+(doneCollapsed?" is-done-collapsed":"")+'"><div class="week-view-head"><div class="week-view-top"><div class="week-view-title-group"><h4 class="week-view-title">\u4efb\u52a1\u6982\u89c8</h4></div><div class="week-view-progress"><span class="week-view-progress-label">\u8fbe\u6210\u7387 <b>'+pct+'%</b></span><span class="week-view-progress-track"><span class="week-view-progress-fill" style="width:'+pct+'%"></span></span></div></div><div class="week-view-stats"><span class="week-view-stat"><b>'+totalAll+'</b>\u9879</span><span class="week-view-stat"><b>'+pendingAll+'</b>\u5f85\u529e</span>'+doneStatHtml+'</div>'+dayStripHtml+'</div><div class="week-day-grid">'+gridInner+'</div></div>';
