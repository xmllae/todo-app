...
function toggleFreeze(id){const t=(T[sel]||[]).find(x=>x.id===id);if(!t)return;const next=!t.frozen;if(typeof setTaskFrozenState==="function")setTaskFrozenState(t,next,sel);else{t.frozen=next;if(t.frozen)t.frozenUntil=""}if(typeof rCal==="function")rCal();if(typeof rT==="function")rT();if(typeof rKanban==="function")rKanban();save();if(typeof refreshGlobalSideNav==="function")refreshGlobalSideNav();toast(next?"❄️ 已冷冻":"🔥 已解冻")}
...
function msSelectAll(){(T[sel]||[]).filter(t=>isListedTask(t)).forEach(t=>selectedIds.add(t.id));rT();document.getElementById("msCnt").textContent=`已选 ${selectedIds.size} 条`}
...
function msFreeze(){pushUndo("批量冷冻");(T[sel]||[]).forEach(t=>{if(!selectedIds.has(t.id))return;const next=!t.frozen;if(typeof setTaskFrozenState==="function")setTaskFrozenState(t,next,sel);else{t.frozen=next;if(t.frozen)t.frozenUntil=""}});selectedIds.clear();toggleMultiSelect();if(typeof rCal==="function")rCal();if(typeof rT==="function")rT();if(typeof rKanban==="function")rKanban();save();if(typeof refreshGlobalSideNav==="function")refreshGlobalSideNav();toast("❄️ 已切换冷冻")}
...
function resetAddTaskFormOnly(){var _as=document.querySelector(".add-split");if(_as)_as.classList.remove("add-split-form-open");var w=document.getElementById("addEmbedWrap"),inp=document.getElementById("tIn"),tt=document.getElementById("tTime"),et=document.getElementById("tEndTime"),dur=document.getElementById("durIn"),ps=document.getElementById("pSel");if(inp)inp.value="";if(tt)tt.value="";if(et)et.value="";if(dur)dur.value="";if(ps)ps.value="normal";if(w)w.classList.remove("add-embed-active");if(typeof closeGhostPriorityDropdown==="function")closeGhostPriorityDropdown();if(inp)try{inp.blur()}catch(e){}refreshAddEmbedPrioArc();syncGhostLabelsFromForm();syncAddTaskMainLabel(false)}
...
function addT(){const inp=document.getElementById("tIn"),txt=inp.value.trim();if(!txt)return;if(!T[sel])T[sel]=[];const dur=parseInt(document.getElementById("durIn").value)||0;T[sel].push(mkTask(txt,document.getElementById("pSel").value,document.getElementById("tTime").value,dur));inp.value="";document.getElementById("tTime").value="";var et=document.getElementById("tEndTime");if(et)et.value="";document.getElementById("durIn").value="";rCal();rT();rTagDropdownContent();save();toast("✅ 已添加");hideAddTaskInline()}
