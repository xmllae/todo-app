...
function startEdit(id){cancelDelayedToggleExpand();const prevItem=document.querySelector('#tList .task-item[data-id="'+id+'"]');const hadVisibleIndicator=!!(prevItem&&prevItem.classList&&prevItem.classList.contains("task-item--detail-active"));editingId=id;editingTimeId=null;expandedId=null;subAddComposingId=null;rT();setTimeout(()=>{if(!hadVisibleIndicator)replayTitleEditIndicatorAnim(id);const item=document.querySelector('#tList .task-item[data-id="'+id+'"]');const inp=item&&item.querySelector(".txt-edit")||document.querySelector(".txt-edit");if(inp){inp.focus();try{inp.select()}catch(e){inp.setSelectionRange(0,inp.value.length)}}},30)}
