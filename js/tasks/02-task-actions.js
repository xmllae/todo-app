...
function startEdit(id){cancelDelayedToggleExpand();editingId=id;editingTimeId=null;expandedId=null;subAddComposingId=null;rT();setTimeout(()=>{replayTitleEditIndicatorAnim(id);const item=document.querySelector('#tList .task-item[data-id="'+id+'"]');const inp=item&&item.querySelector(".txt-edit")||document.querySelector(".txt-edit");if(inp){inp.focus();try{inp.select()}catch(e){inp.setSelectionRange(0,inp.value.length)}}},30)}
...
function openSubAddCompose(tid){if(expandedId!==tid)return;subAddComposingId=tid;rT();setTimeout(function(){const inp=document.getElementById("subAdd_"+tid);if(inp){inp.focus();inp.select()}},30)}
function cancelSubAddCompose(tid){const inp=document.getElementById("subAdd_"+tid);if(inp)inp.value="";if(subAddComposingId===tid)subAddComposingId=null;rT()}
function addSubtask(tid){const inp=document.getElementById("subAdd_"+tid);if(!inp)return;const txt=inp.value.trim();if(!txt)return;const t=(T[sel]||[]).find(x=>x.id===tid);if(!t)return;if(!t.subtasks)t.subtasks=[];t.subtasks.push({id:Date.now()+Math.floor(Math.random()*1e4),text:txt,done:false});inp.value="";syncToRule(t);rT();save();setTimeout(function(){const i=document.getElementById("subAdd_"+tid);if(i&&subAddComposingId===tid)i.focus()},30)}
