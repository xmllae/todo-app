...
function batchDone(){flushPendingTogIfAny();pushUndo("全部完成");(T[sel]||[]).filter(t=>!t.frozen&&isListedTask(t)).forEach(t=>{t.done=true;t.status="done"});rCal();rT();save();toast("✅ 全部已标记完成")}
