...
function goToday(){flushPendingTogIfAny();cY=now.getFullYear();cM=now.getMonth();sel=fd(now);expandedId=null;if(typeof clearSubtaskCollapseOverrides==="function")clearSubtaskCollapseOverrides();closeTaskMoreFloat();taskMoreMenuId=null;rCal();rAll()}
function pick(ds){flushPendingTogIfAny();sel=ds;const p=ds.split("-");cY=+p[0];cM=+p[1]-1;expandedId=null;if(typeof clearSubtaskCollapseOverrides==="function")clearSubtaskCollapseOverrides();closeTaskMoreFloat();taskMoreMenuId=null;rCal();rAll();if(window.innerWidth<640)closeSidebar()}
function quickGo(o){if(o===0){goToday();return}flushPendingTogIfAny();const d=parseDS(sel);d.setDate(d.getDate()+o);sel=fd(d);cY=d.getFullYear();cM=d.getMonth();expandedId=null;if(typeof clearSubtaskCollapseOverrides==="function")clearSubtaskCollapseOverrides();closeTaskMoreFloat();taskMoreMenuId=null;rCal();rAll()}
