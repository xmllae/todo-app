...
function toggleSearch(){const w=document.getElementById("searchWrap");w.classList.toggle("open");if(w.classList.contains("open"))document.getElementById("searchIn").focus();else{document.getElementById("searchIn").value="";document.getElementById("searchResults").classList.add("hidden")}}
...
function toggleDark(){isDark=!isDark;document.body.classList.toggle("dark",isDark);setDarkBtnIcon(true);try{localStorage.setItem("tuole_dark",isDark?"1":"0")}catch(e){}try{refreshAddEmbedPrioArc()}catch(e){}}
